                        ...
                        {links.map((link) => {
                            ...
                            return (
                                <a
                                    key={link.href}
                                    href={link.href}
                                    className={`relative px-4 py-2 text-sm font-semibold transition-colors duration-200 rounded-xl ${isActive ? 'text-brand-700' : 'text-slate-500 hover:text-slate-800'
                                        }`}
                                >
                                    {isActive && (
                                        <motion.div
                                    ...
                                    <span className="relative z-10 flex items-center gap-1.5">
                                        {renderIcon(link.icon, "w-4 h-4 text-slate-500")}
                                        {link.label}
                                    </span>
                                </a>
                            );
                                    ...
                                    {userMenuOpen && (
                                        ...
                                        >
                                            {user.menu?.map((item) => (
                                                ...
                                                >
                                                    {renderIcon(item.icon, "w-4 h-4 text-slate-500 group-hover:text-brand-600")}
                                                    {item.label}
                                                </a>
                                            ))}
                    ...
                    >
                        <nav className="container mx-auto flex flex-col gap-1 px-4 py-4">
                            {links.map((link, i) => (
                                ...
                                >
                                    {renderIcon(link.icon, "w-5 h-5 text-slate-500")}
                            ...
                            {user && (
                                <>
                                    ...
                                    {user.menu?.map((item) => (
                                        <a key={item.href} href={item.href} className="flex items-center gap-3 rounded-xl px-4 py-3 text-slate-600 hover:bg-slate-50 transition">
                                            {renderIcon(item.icon, "w-5 h-5 text-slate-500")}
                                            <span className="font-medium">{item.label}</span>
                                        </a>
                                    ))}
                                    <form action={logoutUrl} method="POST">
                                        <input type="hidden" name="_token" value={csrfToken} />
                                        <button type="submit" className="flex w-full items-center gap-3 rounded-xl px-4 py-3 text-left text-red-600 hover:bg-red-50 transition">
                                            <Icon icon="lucide:log-out" className="w-5 h-5 text-red-500" />
