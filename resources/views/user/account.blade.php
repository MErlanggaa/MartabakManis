                ...
                @if($likedVideos->count() > 0)
                    <div class="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-1">
                        @foreach($likedVideos as $video)
                            <a href="{{ route('videos.show', $video->id) }}?source=liked" class="block relative aspect-[9/16] bg-black rounded-xl overflow-hidden group">
                                <video src="{{ Storage::url($video->video_path) }}" class="w-full h-full object-cover opacity-90 group-hover:opacity-100 transition-opacity" muted loop onmouseover="this.play()" onmouseout="this.pause();this.currentTime=0;"></video>
                               <div class="absolute inset-0 bg-gradient-to-b from-transparent to-black/60 pointer-events-none z-10"></div>
                                
                                <div class="absolute top-2 left-2 flex items-center gap-1 text-white text-xs font-bold drop-shadow-md z-20">
